...
export default function CreateUserForm() {
    ...
    useEffect(() => {
        const fetchBelts = async () => {
            try {
                const response = await fetch('http://localhost:3001/api/public/get/belt')
                const data = await response.json()
                setBelts(data.data)
                // Set default belt_id to first belt
                if (data.data.length > 0) {
                    setFormData(prev => ({ ...prev, belt_id: data.data[0].id.toString() }))
                }
            } catch (err) {
        ...
        <Card className="w-full max-w-4xl mx-auto border-border/50 shadow-sm pt-0">
            <CardHeader className="bg-muted/30 border-b border-border/50 pb-8 pt-8">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <CardTitle className="text-2xl font-bold">Buat Pengguna Baru</CardTitle>
                ...
                <div className="mb-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        {(['murid', 'pelatih', 'admin'] as const).map(r => (
                            <label
                                key={r}
                                className={`relative flex flex-col items-center justify-center p-4 rounded-xl border-2 cursor-pointer transition-all duration-200 ${role === r
                ...
                <form id="create-user-form" onSubmit={handleSubmit} className="space-y-6">
                    ...
                    {(role === 'murid' || role === 'pelatih') && (
                        <div className="p-5 rounded-xl border border-border/50 bg-muted/20 space-y-4">
                            <h3 className="font-medium text-foreground">Informasi Taekwondo</h3>
                            <div className="grid gap-6 md:grid-cols-2">
                                <div className="space-y-2">
                                    <Label htmlFor="belt_id">Sabuk Saat Ini</Label>
                                    <Select
                                        value={formData.belt_id}
                                        onValueChange={(value) => handleSelectChange('belt_id', value)}
                                    >
                                        <SelectTrigger className="h-11">
                                        ...
                                        <SelectContent>
                                            {belts.map(belt => (
                                                <SelectItem key={belt.id} value={belt.id.toString()}>
                                                    {belt.name} {belt.dan_level ? `(DAN ${belt.dan_level})` : ''}
                                                </SelectItem>
                                            ))}
                                        </SelectContent>
                                    </Select>
                                    ...
                                    <>
                                        <div className="space-y-2">
                                            ...
                                            <Select
                                                ...
                                                onValueChange={(value) => handleSelectChange('spesialisasi', value)}
                                            >
                                                <SelectTrigger className="h-11">
                                                ...
                                                <SelectContent>
                                                    ...
                                                    <SelectItem value="kyorugi & poomsae">Kyorugi & Poomsae</SelectItem>
                                                </SelectContent>
                                            </Select>
                                        </div>
                                        <div className="space-y-2 md:col-span-2">
                                            <Label htmlFor="sertifikasi">
                                                Sertifikasi <span className="text-muted-foreground font-normal">(opsional)</span>
                                            </Label>
                                            <Input
                                                id="sertifikasi"
                                                name="sertifikasi"
