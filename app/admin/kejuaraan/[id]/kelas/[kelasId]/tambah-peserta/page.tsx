...
export default function AddParticipantsToClassPage() {
  ...
  useEffect(() => {
    if (!kejuaraanId || !kelasId) return;

    const fetchCandidates = async () => {
      try {
        setLoading(true);
        const response = await fetch(
          `/api/admin/kejuaraan/${kejuaraanId}/kelas-kejuaraan/${kelasId}/calon-peserta?page=1&limit=100`,
        );
  ...
  const handleAddParticipants = async () => {
    if (selectedParticipants.size === 0 || !kejuaraanId || !kelasId) return;
    try {
      setSubmitting(true);
      const response = await fetch(
        `/api/admin/kejuaraan/${kejuaraanId}/kelas-kejuaraan/${kelasId}/peserta/bulk`,
        {
          method: "POST",
  ...
  const handleCancel = () => {
    if (kejuaraanId) router.push(`/admin/kejuaraan/${kejuaraanId}/kelas`);
    else router.back();
  };

  if (!kejuaraanId || !kelasId) {
    return (
      <SidebarProvider
        style={
          {
        ...
        <SidebarInset>
          ...
          <div className="flex flex-1 flex-col items-center justify-center">
            ...
            <Button variant="outline" className="mt-4" asChild>
              <Link href="/admin/kejuaraan">Kembali ke Daftar Kejuaraan</Link>
            </Button>
          </div>
        </SidebarInset>
      </SidebarProvider>
            ...
            <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between border-b pb-5">
              <div className="space-y-1">
                ...
                <p className="text-sm text-muted-foreground">
                  Pilih satu atau lebih murid untuk didaftarkan ke dalam kelas
                  ini
                </p>
              </div>
