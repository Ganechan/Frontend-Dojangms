...
interface KyorugiClassInChampionship {
...
export default function KyorugiClassesInChampionshipPage() {
  const params = useParams();
  const router = useRouter();
  const kejuaraanId = params.id as string;
  const [classes, setClasses] = useState<KyorugiClassInChampionship[]>([]);
  ...
  const fetchClasses = async () => {
    try {
      ...
      const response = await fetch(
        `/api/admin/kejuaraan/${kejuaraanId}/kelas-kyorugi?page=1&per_page=100`,
      );
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      toast.success(
        data.message || "Kelas kyorugi berhasil dihapus dari kejuaraan",
      );
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold">
                  Kelas Kyorugi dalam Kejuaraan
                </h1>
                <p className="text-sm text-muted-foreground">
                  Daftar kelas kyorugi yang telah ditambahkan ke kejuaraan ini
                </p>
              </div>
            </div>
                ...
                <TableBody>
                  {classes.length === 0 ? (
                    <TableRow>
                      ...
                      >
                        Belum ada kelas kyorugi yang ditambahkan
                      </TableCell>
                    </TableRow>
                  ) : (
                    classes.map((cls) => (
                      <TableRow key={cls.kelas_kejuaraan_id}>
                        ...
                        <TableCell>
                          {cls.batas_bawah} - {cls.batas_atas}
                        </TableCell>
                        <TableCell>
