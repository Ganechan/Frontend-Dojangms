...
export default function TambahKelasKyorugiPage() {
    ...
    const fetchClasses = async () => {
      try {
        ...
        const response = await fetch(
          `/api/admin/kejuaraan/${kejuaraanId}/kelas-kyorugi/available?page=1&per_page=100`,
        );
    ...
    try {
      const response = await fetch(
        `/api/admin/kejuaraan/${kejuaraanId}/kelas`,
        {
          ...
          body: JSON.stringify({ tipe: "kyorugi", kelas_id: kelasId }),
        },
      );
      const data = await response.json();
      if (!response.ok)
        throw new Error(data.message || "Gagal menambahkan kelas");
      toast.success("Kelas kyorugi berhasil ditambahkan ke kejuaraan");
        ...
        <div className="flex flex-1 flex-col p-6 bg-background">
          <div className="max-w-6xl mx-auto w-full space-y-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold">Tambah Kelas Kyorugi</h1>
                <p className="text-sm text-muted-foreground">
                  Pilih kelas kyorugi yang akan ditambahkan ke kejuaraan ini
                </p>
              </div>
            </div>
                ...
                <TableBody>
                  {classes.length === 0 ? (
                    <TableRow>
                      ...
                      >
                        Tidak ada kelas kyorugi yang tersedia
                      </TableCell>
                    </TableRow>
                  ) : (
                    classes.map((cls) => (
