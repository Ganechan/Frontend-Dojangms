...
// Interface untuk data kelas kyorugi
interface KyorugiClass {
  id: number;
  gender: string;
  label: string;
  batas_bawah: number;
...
export default function KyorugiClassesPage() {
  const [classes, setClasses] = useState<KyorugiClass[]>([]);
  ...
  const fetchClasses = async (page: number = 1) => {
    try {
      ...
      const response = await fetch(
        `/api/admin/kelas-kyorugi?page=${page}&per_page=10`,
      );
      const data: KyorugiResponse = await response.json();
      ...
      console.error("Error fetching kyorugi classes:", error);
      toast.error(error.message || "Gagal memuat data kelas kyorugi");
    } finally {
      setLoading(false);
    }
  };
  ...
  // Delete handler (jika diperlukan)
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const response = await fetch(
        `/api/admin/kelas-kyorugi/delete/${deletingId}`,
        {
          method: "DELETE",
        },
      );
      ...
      toast.success(data.message || "Kelas kyorugi berhasil dihapus");
  ...
  return (
    ...
    >
            ...
            <div className="flex flex-col gap-4 py-4 md:gap-6 md:py-6">
              <div className="min-h-screen bg-background p-6">
                <div className="max-w-7xl mx-auto">
                  ...
                  <div className="mb-8 flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-muted-foreground">
                        Kelola kelas pertandingan Kyorugi berdasarkan berat
                        badan, usia, dan level
                      </p>
                    </div>
                    <Link href="/admin/kelas-kyorugi/create">
                      <Button>
                  ...
                  {/* Summary Cards (opsional, bisa dihilangkan jika tidak ada summary) */}
                  ...
                  <div className="bg-card rounded-lg border border-border p-4 mb-6 space-y-4">
                    <div className="relative">
                      ...
                      <Input
                        type="text"
                        placeholder="Cari label kelas (contoh: under-42)..."
                        value={searchTerm}
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow className="hover:bg-transparent">
                          <TableHead>Label Kelas</TableHead>
                          <TableHead>Gender</TableHead>
                          <TableHead>Batas Bawah (kg)</TableHead>
                          <TableHead>Batas Atas (kg)</TableHead>
                      ...
                      <TableBody>
                        {loading ? (
                          ...
                          <TableRow>
                            ...
                            >
                              Tidak ada kelas kyorugi yang ditemukan
                            </TableCell>
                          </TableRow>
                        ) : (
                            ...
                            <TableRow key={cls.id}>
                              ...
                              <TableCell className="text-right">
                                <div className="flex gap-2 justify-end">
                                  <Link href={`/admin/kelas-kyorugi/${cls.id}`}>
                                    <Button size="sm" variant="ghost">
                                      <Eye className="w-4 h-4" />
                                    </Button>
                                  </Link>
                                  <Link
                                    href={`/admin/kelas-kyorugi/${cls.id}/edit`}
                                  >
                                    <Button size="sm" variant="ghost">
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Hapus Kelas Kyorugi</AlertDialogTitle>
            <AlertDialogDescription>
